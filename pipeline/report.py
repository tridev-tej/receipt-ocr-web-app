...
def generate_markdown_report(
    ...
    lines.append("| | Claude Vision | Gemini Flash (fallback) |")
    lines.append("|---|---|---|")
    lines.append("| Structured output | Tool-use JSON schema | JSON mode |")
    lines.append("| Multi-language | 7 languages native | 40+ languages |")
    lines.append("| Confidence source | Per-field (high/med/low) | Per-field |")
    lines.append("| Avg confidence | 0.95+ (structured) | 0.90+ (structured) |")
    lines.append("| Cost | ~$0.03/receipt | ~$0.003/receipt |")
    lines.append("| Latency | ~2-3s/receipt | ~1-2s/receipt |")
    lines.append("| Handles rotation | Yes (vision model) | Yes (vision model) |")
    lines.append("")

    # Methodology
    lines.append("## Methodology")
    lines.append("- **OCR**: Claude Vision (primary) with Gemini Flash fallback")
